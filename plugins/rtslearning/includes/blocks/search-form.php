...
function up_search_form_render_cb($atts) {
    ...
    ?>
        <div style="<?php echo $styleAttr; ?>"
            class="wp-block-rtslearning-search-form">
            <h1><?php esc_html_e('Search', 'rtslearning'); ?>:
            <?php the_search_query(); ?>
            </h1>
            <form action="<?php echo esc_url(home_url('/')); ?>">
            <input 
            type="text" 
            placeholder="<?php esc_html_e('Search', 'rtslearning'); ?>" 
            name="s" 
            value="<?php the_search_query(); ?>"
            />
            <div class="btn-wrapper">
                <button type="submit" style="<?php echo $styleAttr; ?>">
                <?php esc_html_e('Search', 'rtslearning'); ?>
            </button>
            </div>
            </form>
        </div>
